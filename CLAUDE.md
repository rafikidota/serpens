...
- `.vscode/` — opt-in editor config: `recommended-settings.json` is tracked and must be copied (or symlinked) to `.vscode/settings.json`, which is git-ignored so personal settings never land in the repo. Also holds `extensions.json` (eslint, prettier, vitest, gitlens), `tasks.json` (build/typecheck/lint/test + a `verify` task chaining `prepublishOnly`'s order, with `$tsc`/`$eslint-stylish` problem matchers) and `README.md` with the setup steps. The recommended settings enable Explorer file nesting so `package.json`, `tsconfig.json`, `.gitignore`, `README.md` and each `index.ts` barrel absorb their related files.
